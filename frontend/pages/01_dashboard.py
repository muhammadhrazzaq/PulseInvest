...
def get_watchlist():
    try:
        r = requests.get(
            f"{API_URL}/portfolio/watchlist", headers=get_headers(), timeout=10
        )
        return r.json()
    except Exception as e:
...
def get_trades():
    try:
        r = requests.get(
            f"{API_URL}/portfolio/trades", headers=get_headers(), timeout=10
        )
        return r.json()
    except Exception:
        return []
...
with st.form("trade_form"):
    ...
    notes = st.text_input(
        "Notes (optional)", placeholder="Why are you making this trade?"
    )
    ...
    if watchlist:
        for item in watchlist:
            ...
            if col4.button("❌", key=f"del_{item['ticker']}"):
                requests.delete(
                    f"{API_URL}/portfolio/watchlist/{item['ticker']}",
                    headers=get_headers(),
                )
                st.rerun()
    else:
